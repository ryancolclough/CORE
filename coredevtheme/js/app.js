...
const PLATFORM = {
  version:"3.2.0-premium-committee-workflow",
  build:"20260719.420",
    ...
    <div class="app-shell">
      ...
      <header class="topbar">
        <button class="brand" data-route="dashboard" aria-label="CORE home">
          <img class="core-header-logo" src="assets/branding/core-header-logo.png?v=20260719.420" alt="CORE — Compliance & Organizational Resource Engine">
        </button>
...
async function boot(){
  await pwa.init();
  const registry = await fetch("data/module-registry.json?v=20260719.420", {cache:"no-store"}).then(r=>{
    if(!r.ok) throw new Error(`Module registry HTTP ${r.status}`);
    return r.json();
  });
  for(const item of registry.filter(x=>x.enabled)){
    const mod = await import(`${item.entry}?v=20260719.420`);
